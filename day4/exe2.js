...
function rechercherProduits(catalogue){
     choix = prompt(`
    1 - recherche avec nom
    2 - Rechercher avec maximum de prix ou minimum
    3 - Recherch dans le stock
    choise:
`);
    switch(choix){
      case 1:
        let nomRech=prompt(`nom : `);
        let cat =catalogue.filter(c => c.nom==nomRech);
        console.log(cat);
        break;
      ...
      case 2:
            ...
            let max=prompt(`max : `);
            let min=prompt(`min : `);
            let maxcat=catalogue.filter(c=>c.prix<max && c.prix>min);
            console.log(maxcat);
        break;  
